...
std::string ToHexString(const unsigned char *p, std::size_t size)
{
...
int main(int argc, char **argv)
{
    ...
    std::cout << Base32Decode("965p604xzdrffvg90ax9bk0q3xyqn5zz2vc9zpbe3wdswzazj7d144mm", bin) << std::endl;
    std::cout << ToHexString(bin, 32) << std::endl;
    return 0;
}
*/

// g++ -o libbbc.so -shared -fPIC bbc.cpp
extern "C" bool Addr2Hex(const char *addr, char *hex)
{
    unsigned char bin[33] = {0};
    bin[0] = addr[0] - '0';
    bool b = Base32Decode(&addr[1], &bin[1]);
    if (b)
    {
        std::string str = ToHexString(bin, sizeof(bin));
